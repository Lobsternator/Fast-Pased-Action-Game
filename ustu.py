...
def RandFunc(chance, func):
    "Calls a function at a given chance."
    chance = 1 - chance
...
def PathExtend(extension):
    ...
    if os.path.dirname(__file__) != "":
        ...
        return os.path.join(os.path.dirname(__file__), f"{extension}")
    else:
        for i, char in enumerate(extension):
            if char == "/":
                extension = list(extension)
        ...
        return os.path.join(os.path.dirname(sys.executable), f"{extension}")
